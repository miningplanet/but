    ...
    platformStyle(_platformStyle)
{
    ...
    {
        progressBar->setStyleSheet("QProgressBar { background-color: #green; border: 1px solid grey; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 green, stop: 1 green); border-radius: 7px; margin: 0px; }");
    }
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a But address"));
    ...
    privateSendCoinsAction->setStatusTip(tr("PrivateSend coins to a Butk address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and butk: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about But Core Gold"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your But addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified But addresses"));
    ...
    openAction->setStatusTip(tr("Open a butk: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible But command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    ...
    if(walletFrame)
    {
        ...
        // Add But logo on the right side
        QWidget* spacer = new QWidget();
        spacer->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        toolbar->addWidget(spacer);

        QLabel *logoLabel = new QLabel();
        QPixmap logoPixmap(":/images/but_logo_toolbar");
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to But network", "", count));
    } else {
...
{
    QString strTitle = tr("But Core Gold"); // default title
